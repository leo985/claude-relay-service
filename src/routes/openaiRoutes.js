...
function buildAllUpstreamRateLimitedPayload(resetsInSeconds = null) {
  const error = {
...
function sendAllUpstreamRateLimitedResponse(res, { resetsInSeconds = null, stream = false } = {}) {
  const payload = buildAllUpstreamRateLimitedPayload(resetsInSeconds)
  if (resetsInSeconds !== null && resetsInSeconds !== undefined) {
...
function canRetryOpenAIUpstreamRateLimit(apiKeyData = {}) {
  const binding = apiKeyData.openaiAccountId || ''
  return !binding || binding.startsWith('group:')
}

function getOpenAIRateLimitRetryCount(req) {
  return Number.isInteger(req._openaiRateLimitRetryCount) ? req._openaiRateLimitRetryCount : 0
}

function addOpenAIRateLimitFailedAccount(req, accountId) {
  const failedAccountIds = Array.isArray(req._openaiRateLimitFailedAccountIds)
    ? req._openaiRateLimitFailedAccountIds.filter(Boolean)
    : []
  const alreadyFailed = Boolean(accountId && failedAccountIds.includes(accountId))
  req._openaiRateLimitFailedAccountIds = [
    ...new Set([...failedAccountIds, accountId].filter(Boolean))
  ]
  return {
    alreadyFailed,
    failedAccountIds: req._openaiRateLimitFailedAccountIds
  }
}
...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    const failedOpenAIAccountIds = Array.isArray(req._openaiRateLimitFailedAccountIds)
      ? req._openaiRateLimitFailedAccountIds.filter(Boolean)
      : []
    ...
    if (failedOpenAIAccountIds.length > 0) {
      requestFeatures.excludeAccountIds = failedOpenAIAccountIds
    }
    ...
    if (upstream.status === 429) {
      ...
      const retryCount = getOpenAIRateLimitRetryCount(req)
      const { alreadyFailed, failedAccountIds } = addOpenAIRateLimitFailedAccount(req, accountId)
      const canRetryRateLimit =
        canRetryOpenAIUpstreamRateLimit(apiKeyData) &&
        Boolean(accountId) &&
        !alreadyFailed &&
        !res.headersSent &&
        !res.writableEnded &&
      ...
      if (canRetryRateLimit) {
        req._openaiRateLimitRetryCount = retryCount + 1
        if (sessionHash) {
          await unifiedOpenAIScheduler._deleteSessionMapping(sessionHash).catch(() => {})
        }
        logger.warn(
          `🔁 Retrying OpenAI Codex request after upstream 429; excluded ${failedAccountIds.length} failed account(s)`
        )
        return await handleResponses(req, res)
      }

      sendAllUpstreamRateLimitedResponse(res, { resetsInSeconds, stream: isStream })
      return
    ...
    if (!res.headersSent) {
      if (getOpenAIRateLimitRetryCount(req) > 0 && status === 402) {
        return sendAllUpstreamRateLimitedResponse(res)
      }
      if (status === 429) {
        const resetsInSeconds =
          responsePayload?.error?.resets_in_seconds ||
          upstreamErrorHelper.parseRetryAfter(error.response?.headers)
        return sendAllUpstreamRateLimitedResponse(res, { resetsInSeconds })
      }
...
const handleImageGenerations = async (req, res) => {
  try {
    ...
    if (error.statusCode === 429 || error.response?.status === 429) {
      return sendAllUpstreamRateLimitedResponse(res, {
        resetsInSeconds: upstreamErrorHelper.parseRetryAfter(error.response?.headers)
      })
    }
